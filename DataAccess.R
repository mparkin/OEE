...
getResourcedata = function(startDate, Res, endTime)
{
  ...
  sqlcmd <- paste("SELECT Resource, E10, E58, Reason, Availability, StartTime, EndTime FROM DW.dbo.f_ResourceHistory WHERE ResourceType = '", Res , "' AND StartTime >= '", startDate,"' AND StartTime <= '", ymd_hms(endTime,tz= Sys.getenv("TZ")) ,"'", sep = "" )
